...
//---------------------------------------------------------------------------
 ...
 cmd({
	...
	const daily  = await eco.daily(citel.sender, VISHWAMD, 2000); //give 500 for daily, can be changed
 ...
 cmd({
	...
	if(!users) return citel.reply('Please give me user.')
        const balance  = await eco.balance(users, "VISHWA-MD")
...
async(Void, citel, text,{ isCreator }) => {
    ...
    switch (value) {
        ...
        if (k > balance.wallet ) return citel.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
          const deduct1 = await eco.deduct(user, VISHWAMD, 100);
          const add1 = eco.giveCapacity(user, VISHWAMD, 1000);
return await citel.reply(`*1000 🪙diamond storage has been added in ${citel.pushName} bank*`)
        ...
        if (k < balance.wallet) return citel.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
          const deduct2 = await eco.deduct(user, VISHWAMD, 1000);
          const add2 = eco.giveCapacity(user, VISHWAMD, 100000);
return await citel.reply(`*100000 🪙diamond storage has been added in ${citel.pushName} bank*`)
        ...
        if (k < balance.wallet) return citel.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
           const deduct3 = await eco.deduct(user, VISHWAMD, 10000);
           const add3 = eco.giveCapacity(user, VISHWAMD, 10000000);
 ...
 await citel.reply('*What are you trying to do📉*.')
    ...
    async(Void, citel, text,{ isCreator }) => {
      ...
      //  let users = citel.mentionedJid ? citel.mentionedJid[0] : citel.msg.contextInfo.participant || false;
        ...
        let d = parseInt(text)
        const deposit = await eco.deposit(citel.sender, "VISHWAMD", d);
        const balance = await eco.balance(citel.sender, "VISHWAMD")
...
return await citel.reply(`⛩️ Sender: ${citel.pushName}\n🍀Successfully 💰Deposited 🪙${deposit.amount} to your bank.Upgrade your bank capacity to add more money📈.`)
    ...
    async(Void, citel, text,{ isCreator }) => {
	let h = await eco.lb('VISHWAMD',10);
...
async(Void, citel, text,{ isCreator }) => {
    ...
    const secktor = "secktor"
        ...
        const balance = await eco.balance(user1, VISHWAMD);
        let a = (balance.wallet) < parseInt(word)
        //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
        if(a == true) return citel.reply("you dont have sufficient money to transfer👎");

        const deduct = await eco.deduct(user1, VISHWAMD, value[0]);
        const give = await eco.give(user2, VISHWAMD, value[0]);
...
return await citel.reply( `*📠 Transaction successful of ${value[0]} 💰*`)
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
         const secktor = "secktor"
         const balance = await eco.balance(citel.sender, VISHWAMD); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
...
return await citel.reply(`🍀User: ${citel.pushName}\n\n_🪙${balance.bank}/${balance.bankCapacity}_`)
	...
	const secktor = "secktor"
	    const k = 1000
        const balance1  = await eco.balance(user1, VISHWAMD)
	const balance2  = await eco.balance(user2, VISHWAMD)
    ...
    switch (random) {
	  ...
	  const deduff = Math.floor(Math.random() * 1000)	    
          await eco.deduct(user2, VISHWAMD, deduff);
          await eco.give(citel.sender, VISHWAMD, deduff);
        ...
        case 'caught':
           ...
           await eco.deduct(user1, VISHWAMD, rmoney);
...
default:
 ...
 //citel.react('🤔')
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        const secktor = "secktor"
        const withdraw = await eco.withdraw(user, VISHWAMD, query);
 ...
 ///.mentionedJid[0] ? m.mentionedJid[0] : m.sender
     const secktor = "secktor"
     const balance = await eco.balance(user, VISHWAMD);
   ...
   citel.reply(media,{packname:'VISHWAMD',author:'Economy'},"sticker")
     ...
     if (!text) return citel.reply(
        ...
        if ( r == opp){
           let give = await eco.give(user , VISHWAMD, twice);
    //citel.react('⭐️')
return await citel.reply( `*📈 You won 🪙${twice}*`)
        }
        else{
           let deduct = await eco.deduct(user, VISHWAMD, texts[0]);
    //citel.react('🤮')
...
if(value<=balance.wallet){
    ...
    if ((f1 !== f2) && f2 !== f3){
        const deduct1 = await eco.deduct(user, VISHWAMD, deduff);
        return citel.reply(`${mess1}\n\n*Big Lose -->* _🪙${deduff}_`)
     }
     else if ((f1 == f2) && f2 == f3){
        const give1 = await eco.give(user, VISHWAMD, deduff/2);
        return citel.reply(`${mess2}\n*_Little Jackpot -->* _🪙${deduff/2}_`)
     }
     else if ((f1 == f2) && f2 !== f3){
        const give2 = await eco.give(user, VISHWAMD, deduff);
        return citel.reply(`${mess3}\n*Small Win -->* _🪙${deduff}_`)
     }
     else if ((f1 !== f2) && f1 == f3){
        const deduct2 = await eco.deduct(user, VISHWAMD, deduff);
        return citel.reply(`${mess5}\n\n*Small Lose -->* _🪙${deduff}_`)
     }
     else if ((f1 !== f2) && f2 == f3){
        const give4 = eco.give(user, VISHWAMD, deduff);
        return citel.reply(`${mess3}\n\n*Small Win -->* _🪙${deduff}_`)
     }
     else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
        const give5 = eco.give(user, VISHWAMD, deduff*20);
    ...
    return citel.reply('You don\'t have enough 💰amount in your👛 wallet.\n- Please don\'t provide 🤑amount.')
            ...
            if ((f1 !== f2) && f2 !== f3){
               const deduct1 = await eco.deduct(user, VISHWAMD, 50);
                      citel.reply(`${mess1}\n\n*Big Lose -->* _🪙50_`)
            }
            else if ((f1 == f2) && f2 == f3){
               const give1 = await eco.give(user, VISHWAMD, 100);
                     citel.reply(`${mess2}\n*_Little Jackpot -->* _🪙100_`)
            }
            else if ((f1 == f2) && f2 !== f3){
               const give2 = await eco.give(user, VISHWAMD, 20);
                     citel.reply(`${mess3}\n*Small Win -->* _🪙20_`)
            }
            else if ((f1 !== f2) && f1 == f3){
               const deduct2 = await eco.deduct(user,VISHWAMD, 20);
                     citel.reply(`${mess5}\n\n*Small Lose -->* _🪙20_`)
            }
            else if ((f1 !== f2) && f2 == f3){
               const give4 = eco.give(user, VISHWAMD, 20);
                     citel.reply(`${mess3}\n\n*Small Win -->* _🪙20_`)
            }
            else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
               const give5 = eco.give(user, VISHWAMD, 1000);
                    citel.reply(`${mess4}\n\n_🎊 JackPot --> _🪙1000_`)
            }
            else {
...
async(Void, citel, text,{ isCreator }) => {
    ...
    if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
    const kg = 100
            const balance1  = await eco.balance(citel.sender, "VISHWAMD")
    ...
    if(lol){
        ...
        const give2 = await eco.give(citel.sender, "VISHWAMD", deduff*2);
    ...
    const deduct1 = await eco.deduct(citel.sender, "VISHWAMD", deduff);
    let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
            let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
